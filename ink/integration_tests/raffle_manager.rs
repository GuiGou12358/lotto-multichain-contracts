...
where
    ...
    <Client as ContractsBackend<DefaultEnvironment>>::Error: Debug,
{
    let set_config = contract.call_builder::<lotto_registration_manager_contract::Contract>()
        .set_config(config);
    client
...
async fn test_raffles<Client: E2EBackend>(mut client: Client) -> E2EResult<()> {
    ...
    assert_eq!(
        true,
        can_close_registrations(&mut client, &contract).await
    );
    ...
    attestor_sends_results_propagated(&mut client, &contract, draw_number, vec![], numbers_hash.clone(), queue_head)
        .await;
    queue_head += 1;
