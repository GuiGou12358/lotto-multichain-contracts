...
pub trait BaseRaffleManager: RaffleManagerStorage + KvStore {
    ...
    fn set_registration_contracts(
        ...
        RaffleManagerStorage::get_mut_storage(self).registration_contracts =
    ...
    fn set_min_number_salts(&mut self, min_number_salts: u8) -> Result<(), RaffleError> {
        // check the status
        self.check_registration_contracts_status(Status::NotStarted)?;
    ...
    fn try_to_generate_salt(
        &mut self,
    ) -> Result<(Option<Salt>, Vec<RegistrationContractId>), RaffleError> {
        ...
        let contracts_salts = RaffleManagerStorage::get_storage(self)
            .salts
            .get(draw_number)
            .unwrap_or_default();
        ...
        if contracts_salts.len() >= min_number_salts {
            ...
            // collect all salt
            let salts: Vec<_> = contracts_salts
                .iter()
                .map(|(_contract, salt)| salt)
                .collect();
            // generate the salt
        ...
        for i in 0..RaffleManagerStorage::get_storage(self)
    ...
    fn generate_salt(
        ...
        match RaffleManagerStorage::get_storage(self)
            .generated_salt
            .get(draw_number)
        {
            Some(_) => return Err(ExistingSalt),
            None => RaffleManagerStorage::get_mut_storage(self)
                .generated_salt
                .insert(draw_number, &output_salt.to_vec()),
        };
        ...
        for (contract_id, salt) in contracts_salts.iter() {
                ...
                .get(contract_id)
            {
                Some(Status::RegistrationsClosed) => {
                    ...
                    RaffleManagerStorage::get_mut_storage(self)
                        .registration_contracts_status
                        .insert(contract_id, &Status::WaitingSalt);
                    // add the hash
                    let mut registered_contracts_salts = RaffleManagerStorage::get_storage(self)
                        .salts
                        .get(draw_number)
                        .unwrap_or_default();
                    ...
                    RaffleManagerStorage::get_mut_storage(self)
                        .salts
                        .insert(draw_number, &registered_contracts_salts);
                }
                _ => return Err(IncorrectStatus),
            }
        }
...
mod tests {
    ...
    fn test_start() {
        ...
        assert_eq!(
            contract.set_registration_contracts(vec![1]),
            Err(IncorrectStatus)
        );
    }
    ...
    fn test_generate_salt() {
        ...
        let generated_salt = contract
            .generate_salt(1, &salts)
            .expect("Fail to generate salt");

        let expected_salt: Salt = [
            94, 193, 212, 179, 22, 80, 18, 236, 194, 56, 99, 20, 16, 125, 123, 20, 14, 26, 212, 42,
            96, 187, 51, 110, 129, 113, 120, 162, 223, 50, 36, 79,
        ]
        .to_vec();
    ...
    fn test_set_winners() {
        ...
        assert_eq!(
            contract.set_winners(1, (vec![], vec![])),
            Err(IncorrectStatus)
        );
        ...
        assert_eq!(
            contract.set_winners(0, (vec![], vec![])),
            Err(IncorrectDrawNumber)
        );
        assert_eq!(
            contract.set_winners(2, (vec![], vec![])),
            Err(IncorrectDrawNumber)
        );
    ...
    fn test_set_winners_substrate() {
        ...
        assert_eq!(contract.get_draw_number(), Ok(1));
        assert_eq!(
            contract.get_winners(1),
            Some((vec![address_substrate_1, address_substrate_2], vec![]))
        );
    }
    ...
    fn test_full() {
        ...
        contract
            .save_salts(2, vec![(101, [1u8; 32].to_vec())])
            .expect("Fail to save the salts");
        let contract_salt = contract
            .try_to_generate_salt()
            .expect("Fail to generate salt");
        ...
        contract
            .save_salts(2, vec![(102, [2u8; 32].to_vec())])
            .expect("Fail to save the salts");
        let contract_salt = contract
            .try_to_generate_salt()
            .expect("Fail to generate salt");
